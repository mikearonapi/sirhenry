...
async def portfolio_summary(session: AsyncSession = Depends(get_session)):
    ...
    other_val = sum((h.current_value or 0) for h in holdings if h.asset_class not in ("stock", "etf"))
    crypto_val = sum((c.current_value or 0) for c in cryptos)
    total_val = stock_val + etf_val + other_val + crypto_val
    total_cost = (
    ...
    # Top holdings
    all_items = [
        {"ticker": h.ticker, "name": h.name, "value": h.current_value or 0, "gain_loss_pct": h.unrealized_gain_loss_pct or 0}
        for h in holdings
    ] + [
        {"ticker": c.symbol, "name": c.name, "value": c.current_value or 0, "gain_loss_pct": 0}
        for c in cryptos
    ]
