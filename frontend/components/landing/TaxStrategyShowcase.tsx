    ...
    <MockupFrame title="SirHENRY \u2014 Tax Strategy" dark={false} fadeBottom={false}>
      ...
      <div className="mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {t.strategies.map((s) => (
            <div key={s.title} className="rounded-lg p-3 border border-[#E5E7EB] hover:border-accent/30 transition-colors">
              ...
              <p className="text-accent text-sm font-semibold" style={{ fontFamily: "var(--font-mono)" }}>
                {s.savings}
              </p>
      ...
      <div className="rounded-lg p-3 border border-[#E5E7EB]">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-accent text-xs font-medium" style={{ fontFamily: "var(--font-mono)" }}>
            {t.checklist.completed}/{t.checklist.total}
          </span>
        </div>
        <div className="w-full h-2 bg-[#F3F4F6] rounded-full overflow-hidden">
          <div className="h-full bg-accent rounded-full" style={{ width: `${checklistPct}%` }} />
        </div>
      </div>
    </MockupFrame>
  );
