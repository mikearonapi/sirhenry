    ...
    <div className="space-y-5">
      ...
      <Card padding="lg">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-text-primary">Documents on File ({yearDocs.length})</h3>
        </div>
        {yearDocs.length === 0 ? (
          <p className="text-sm text-text-muted text-center py-4">No tax documents uploaded for {year}.</p>
        ) : (
          <div className="space-y-1.5">
            {yearDocs.map((doc) => (
              <div key={doc.id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-surface text-sm">
                <div className="flex items-center gap-3">
                  <FileText size={16} className="text-text-muted flex-shrink-0" />
                  <div>
                    <p className="font-medium text-text-secondary truncate max-w-xs">{doc.filename}</p>
                    <p className="text-xs text-text-muted capitalize">{(doc.document_type === "processing" ? "tax document" : doc.document_type).replace(/_/g, " ")} · {doc.status}</p>
                  </div>
                </div>
      ...
      {nonFormItems.length > 0 && (
        <Card padding="lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-text-primary">Filing Readiness</h3>
            <span className="text-sm font-semibold text-text-secondary">{checklist.completed}/{checklist.total} complete</span>
          </div>
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-sm text-text-secondary">Overall Progress</span>
              <span className="text-sm font-semibold text-text-primary">{checklist.progress_pct}%</span>
            </div>
          ...
          {(["documents", "preparation", "filing", "payments"] as const).map((cat) => {
            ...
            return (
              <div key={cat} className="mb-4 last:mb-0">
                <h4 className="text-xs font-semibold text-text-secondary uppercase tracking-wide mb-2">{CATEGORY_LABELS[cat]}</h4>
                <div className="space-y-1.5">
                  {catItems.map((ci) => (
                    <div key={ci.id} className={`flex items-start gap-3 rounded-lg px-3 py-2 text-sm ${
                      ci.status === "complete" ? "bg-green-50/50" : ci.status === "partial" ? "bg-amber-50/50" : ci.status === "not_applicable" ? "bg-surface/50 opacity-50" : "bg-surface/30"
                    }`}>
                      <div className="mt-0.5 flex-shrink-0">
                        {ci.status === "complete" ? <CheckCircle size={16} className="text-green-500" /> :
                         ...
                         ci.status === "not_applicable" ? <MinusCircle size={16} className="text-text-muted" /> :
                         <Circle size={16} className="text-text-muted" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`font-medium ${ci.status === "complete" ? "text-green-800" : ci.status === "not_applicable" ? "text-text-muted" : "text-text-secondary"}`}>{ci.label}</p>
                        {ci.detail && <p className="text-xs text-text-muted mt-0.5">{ci.detail}</p>}
                        {ci.status !== "complete" && ci.status !== "not_applicable" && CHECKLIST_LINKS[ci.id] && (
                          <Link href={CHECKLIST_LINKS[ci.id].href} className="text-xs text-accent hover:underline mt-0.5 inline-block">
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-xs text-accent hover:underline print:hidden"
      >
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full ${colors[status] ?? "bg-surface text-text-secondary"}`}>
      {status}
    </span>
  );
}
