    ...
    return (
      <Card padding="lg">
        <div className="text-center py-8">
          <Calendar size={32} className="mx-auto mb-3 text-text-muted" />
          <p className="text-sm text-text-muted">No upcoming vesting events</p>
          <p className="text-xs text-text-muted mt-1">Add vesting schedules to your grants to see upcoming vests here.</p>
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card padding="lg">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-text-primary">Vesting Timeline (24 months)</h3>
          ...
          return (
            <div key={monthKey}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-semibold text-text-secondary font-mono tabular-nums">{formatCurrency(monthTotal)}</span>
              </div>
              <div className="space-y-2">
                {monthEvents.map((ev) => {
                  ...
                  return (
                    <div key={ev.id} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-surface hover:bg-surface-hover transition-colors">
                      <div className="w-2 h-8 rounded-full" style={{ backgroundColor: color }} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-text-primary">{ev.employer}</span>
                          ...
                          {ev.ticker && <span className="text-xs text-text-muted">{ev.ticker}</span>}
                        </div>
