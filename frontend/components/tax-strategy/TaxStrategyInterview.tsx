...
function Checkbox({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center gap-2 cursor-pointer">
      <input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)} className="rounded border-border text-accent focus:ring-accent" />
...
function Radio({ label, value, selected, onChange }: { label: string; value: string; selected: boolean; onChange: (v: string) => void }) {
  return (
    <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${selected ? "border-accent bg-accent-light/30" : "border-border hover:border-border"}`}>
      <input type="radio" checked={selected} onChange={() => onChange(value)} className="text-accent focus:ring-accent" />
      <span className="text-sm text-text-secondary">{label}</span>
    </label>
  );
}
    ...
    <div className="bg-card rounded-xl border border-card-border shadow-sm">
      ...
      <div className="px-6 pt-5 pb-3">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-text-primary">Tax Strategy Interview</h3>
          <span className="text-xs text-text-muted ml-auto">Step {step + 1} of {STEPS.length}</span>
        </div>
        ...
        <p className="text-xs text-text-secondary mt-2">{STEPS[step]}</p>
      </div>
      ...
      <div className="px-6 py-4 min-h-[260px]">
        {step === 0 && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-text-secondary mb-2">Income Type</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-text-secondary mb-1">Combined Household Income</label>
              ...
              <div>
                <label className="block text-xs text-text-secondary mb-1">Investment Income</label>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-text-secondary mb-2">Last Year Tax Result</label>
              <div className="grid grid-cols-3 gap-2">
        ...
        {step === 2 && (
          <div className="space-y-3">
            <p className="text-xs text-text-secondary mb-2">Check all that apply:</p>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-text-secondary mb-2">Top Priority</label>
              <div className="grid grid-cols-3 gap-2">
            ...
            <div>
              <label className="block text-xs text-text-secondary mb-2">Complexity Comfort Level</label>
              <div className="grid grid-cols-3 gap-2">
        ...
        {step === 4 && (
          <div className="space-y-3">
            <p className="text-sm text-text-secondary mb-3">Review your answers. <SirHenryName /> will use this to generate personalized tax strategies.</p>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="bg-surface rounded-lg p-3">
                <p className="text-xs text-text-secondary mb-1">Income</p>
                <p className="font-medium text-text-primary font-mono tabular-nums">${(profile.combined_income || 0).toLocaleString()}</p>
                <p className="text-xs text-text-secondary mt-1">{profile.income_type === "w2" ? "W-2" : profile.income_type === "self_employed" ? "Self-employed" : "Mixed"} · {profile.filing_status === "mfj" ? "Filing Jointly" : profile.filing_status === "mfs" ? "Filing Separately" : profile.filing_status === "hh" ? "Head of Household" : "Single"}</p>
              </div>
              <div className="bg-surface rounded-lg p-3">
                <p className="text-xs text-text-secondary mb-1">Tax Situation</p>
              ...
              <div className="bg-surface rounded-lg p-3">
                <p className="text-xs text-text-secondary mb-1">Goals</p>
                <p className="text-xs text-text-secondary">
      ...
      <div className="px-6 pb-5 flex items-center justify-between">
        ...
        {step < STEPS.length - 1 ? (
          <button
            type="button"
            onClick={() => setStep(step + 1)}
            className="flex items-center gap-1.5 bg-accent text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-accent-hover"
          >
