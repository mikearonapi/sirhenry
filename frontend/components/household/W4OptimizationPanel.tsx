...
export default function W4OptimizationPanel({ profile, onError }: W4OptimizationPanelProps) {
  ...
  return (
    <Card padding="lg">
      <h3 className="text-sm font-semibold text-text-primary mb-1">W-4 Withholding Optimization</h3>
      ...
      <div className="flex items-center gap-2 mb-4">
        {benefitsLoaded ? (
          <div className="flex items-center gap-1.5 text-xs text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-950/40 px-3 py-1.5 rounded-lg border border-green-100 dark:border-green-900">
            <CheckCircle2 size={12} />
            Pre-tax deductions auto-filled from Benefits tab (401k + health + dental + commuter)
          </div>
        ) : (
          <button onClick={autoFillFromBenefits}
            className="flex items-center gap-1.5 text-xs text-accent bg-accent/5 hover:bg-accent/10 px-3 py-1.5 rounded-lg border border-accent/20 font-medium transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <p className="text-xs font-semibold text-text-secondary mb-2">{profile.spouse_a_name || "Spouse A"}</p>
          <div className="space-y-2">
            <div>
              <label className="text-xs text-text-muted">Pay Frequency</label>
            ...
            <div>
              <label className="text-xs text-text-muted">Pre-tax Deductions — annual (401k + health + dental + commuter)</label>
        ...
        <div>
          <p className="text-xs font-semibold text-text-secondary mb-2">{profile.spouse_b_name || "Spouse B"}</p>
          <div className="space-y-2">
            <div>
              <label className="text-xs text-text-muted">Pay Frequency</label>
      ...
      <div className="mb-4">
        <div className="flex items-center gap-2">
          <label className="text-xs text-text-muted">Other Income — Trust K-1, rental, 1099 (annual, no W-2 withholding)</label>
      ...
      {result && (
        <div className="mt-4 space-y-4">
          ...
          {result.estimated_shortfall > 500 && (
            <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-100 dark:border-amber-900 rounded-xl">
              <p className="text-sm font-semibold text-amber-800 dark:text-amber-400 mb-2">Recommended W-4 Adjustments (Step 4c)</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="p-3 bg-card rounded-lg border border-amber-100 dark:border-amber-900">
                  <p className="text-xs text-text-secondary">{profile.spouse_a_name || "Spouse A"} — Additional Per Paycheck</p>
                  <p className="text-xl font-bold text-amber-700">{formatCurrency(result.extra_per_paycheck_a)}</p>
                </div>
                <div className="p-3 bg-card rounded-lg border border-amber-100 dark:border-amber-900">
                  <p className="text-xs text-text-secondary">{profile.spouse_b_name || "Spouse B"} — Additional Per Paycheck</p>
          ...
          {result.estimated_shortfall <= 0 && (
            <div className="p-4 bg-green-50 dark:bg-green-950/40 border border-green-100 dark:border-green-900 rounded-xl flex items-center gap-3">
              <CheckCircle2 size={16} className="text-green-600" />
              <p className="text-sm text-green-800 dark:text-green-400">{result.recommendation}</p>
            </div>
          )}
        </div>
      )}
