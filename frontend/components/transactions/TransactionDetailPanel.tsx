...
export default function TransactionDetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-card shadow-2xl border-l border-border overflow-y-auto">
        <div className="sticky top-0 bg-card border-b border-card-border px-5 py-3 flex items-center justify-between z-10">
          <h2 className="font-semibold text-text-primary text-sm">Transaction Details</h2>
          <button onClick={onClose} aria-label="Close" className="p-1 rounded-lg hover:bg-surface text-text-muted">
            <X size={16} />
          </button>
        </div>

        <div className="px-5 py-4 space-y-4">
          <div className="text-center pb-3 border-b border-card-border">
            <p className={`text-2xl font-bold tracking-tight ${tx.amount >= 0 ? "text-green-600" : "text-text-primary"}`}>
              {tx.amount >= 0 ? "+" : ""}{formatCurrency(tx.amount)}
            </p>
            <p className="text-sm text-text-secondary mt-1 font-medium">{cleanTransactionName(tx.description, tx.merchant_name)}</p>
            ...
            <div className="bg-surface rounded-lg p-3 space-y-1.5 text-xs">
              {tx.description !== cleanTransactionName(tx.description, tx.merchant_name) && (
                <div className="flex justify-between gap-2">
                  <span className="text-text-secondary shrink-0">Bank description</span>
                  <span className="text-text-secondary text-right truncate" title={tx.description}>{tx.description}</span>
                </div>
              )}
              {tx.category && (
                <div className="flex justify-between">
                  <span className="text-text-secondary">Original category</span>
                  <span className="text-text-secondary font-medium">{tx.category}</span>
                </div>
              )}
              {tx.category_override && (
                <div className="flex justify-between">
                  <span className="text-text-secondary">User override</span>
                  <span className="text-violet-600 font-medium">{tx.category_override}</span>
                </div>
              )}
              {confidencePct !== null && (
                <div className="flex justify-between items-center">
                  <span className="text-text-secondary flex items-center gap-1"><Bot size={10} /> AI confidence</span>
              ...
              {tx.data_source && (
                <div className="flex justify-between">
                  ...
                  <span className="text-text-secondary font-medium capitalize">{tx.data_source}</span>
                </div>
              )}
            </div>
          )}
          ...
          <div>
            ...
            <select
              ...
              className={`w-full text-sm border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent bg-card ${
                !category ? "border-amber-300 text-amber-700" : "border-border"
              }`}
            >
          ...
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1">Tax Category</label>
            <select
              value={taxCategory}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-1">Segment</label>
              <select
                value={segment}
            ...
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-1">Business Entity</label>
              <select
                value={entityId ?? ""}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-accent text-white text-sm font-medium py-2 rounded-lg hover:bg-accent-hover disabled:opacity-40 transition-colors shadow-sm"
          >
