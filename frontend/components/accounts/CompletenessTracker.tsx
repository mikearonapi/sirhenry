  ...
  return (
    <div className="bg-card border border-card-border rounded-xl p-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-sm font-semibold text-text-primary">Financial Picture Completeness</p>
          <p className="text-xs text-text-secondary">{complete} of {steps.length} categories connected -- {pct}% complete</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        {steps.map((s) => (
          <div key={s.label} className={`flex items-center gap-2 p-2 rounded-lg text-xs ${s.count > 0 ? "bg-green-50" : "bg-surface"}`}>
            ...
            <div>
              <p className={`font-medium ${s.count > 0 ? "text-green-700" : "text-text-secondary"}`}>{s.label}</p>
              <p className={`text-xs ${s.count > 0 ? "text-green-600" : "text-text-muted"}`}>
