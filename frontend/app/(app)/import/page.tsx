...
const STATUS_ICONS: Record<string, React.ReactNode> = {
  ...
  duplicate: <AlertCircle size={14} className="text-text-muted" />,
};
...
export default function ImportPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-text-primary">Data &amp; Documents</h1>
        <p className="text-text-secondary text-sm mt-0.5">Import data from external sources and manage your document vault</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-card rounded-xl border border-card-border shadow-sm p-5 space-y-4">
            <h2 className="font-semibold text-text-primary">Document Type</h2>
            ...
            <p className="text-xs text-text-muted">{selectedDocType.hint}</p>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-text-secondary mb-1">Account / Issuer Name</label>
                <input
                  type="text"
                  value={accountName}
              ...
              <div>
                <label className="block text-xs text-text-secondary mb-1">Institution</label>
                <input
                  type="text"
                  value={institution}
            ...
            {(docType === "credit_card" || docType === "monarch") && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-text-secondary mb-1">Default Segment</label>
                  <select
                    value={segment}
            ...
            {(docType === "tax_document" || docType === "investment") && (
              <div>
                <label className="block text-xs text-text-secondary mb-1">Tax Year (optional — auto-detected from file)</label>
                <select
                  value={taxYear ?? ""}
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="rounded"
              />
              <label htmlFor="auto-cat" className="text-sm text-text-secondary">
          ...
          >
            ...
            {uploading ? (
              ...
              <div className="flex flex-col items-center gap-3 text-text-muted">
                <Upload size={32} />
                <div>
                  <p className="text-sm font-medium text-text-secondary">Drop files here or click to browse</p>
          ...
          {results.length > 0 && (
            <div className="bg-card rounded-xl border border-card-border shadow-sm p-5">
              <h2 className="font-semibold text-text-primary mb-3">Import Log</h2>
                ...
                {results.map((r, i) => (
                  <div key={i} className="flex items-start gap-3 text-sm">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-text-secondary truncate">{r.filename}</p>
                      <p className="text-text-secondary text-xs mt-0.5">{r.message}</p>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-card rounded-xl border border-card-border shadow-sm p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold text-text-primary text-sm">Run AI Categorization</h2>
            </div>
            ...
            <select
              ...
              className="w-full text-sm border border-border rounded-lg px-3 py-2 mb-3 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent"
            >
          ...
          <div className="bg-card rounded-xl border border-card-border shadow-sm p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-text-primary text-sm">Recent Imports</h2>
              <button onClick={loadDocuments} className="p-1 rounded hover:bg-surface text-text-muted">
                <RefreshCw size={13} />
              </button>
            </div>
            {docsLoading ? (
                ...
                {documents.slice(0, 10).map((doc) => (
                  <div key={doc.id} className="flex items-start gap-2 text-xs">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="truncate font-medium text-text-secondary">{doc.filename}</p>
                      <p className={`${statusColor(doc.status)} capitalize`}>
                        {doc.status} · {formatDate(doc.imported_at)}
                      </p>
                      {doc.tax_year && <p className="text-text-muted">Tax year {doc.tax_year}</p>}
                    </div>
                  </div>
                ))}
                {documents.length > 10 && (
                  <button onClick={() => setActiveTab("vault")} className="text-xs text-accent hover:underline w-full text-center">
      ...
      {activeTab === "vault" && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-48">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent/20"
              />
            </div>
            <select
              value={vaultTypeFilter}
              onChange={(e) => setVaultTypeFilter(e.target.value)}
              className="text-sm border border-border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/20"
            >
            ...
            <button onClick={loadDocuments} className="p-2 text-text-muted hover:text-text-secondary border border-border rounded-lg hover:bg-surface">
              <RefreshCw size={14} />
            </button>
            <span className="ml-auto text-xs text-text-muted">{filteredDocs.length} of {documents.length} documents</span>
          </div>
            ...
            <div className="bg-card rounded-xl border border-card-border overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-surface border-b border-card-border">
                  <tr>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wide">Document</th>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wide">Tax Year</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wide">Imported</th>
                    ...
                    <tr key={doc.id} className="hover:bg-surface transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-text-primary truncate max-w-xs">{doc.filename}</span>
                        </div>
                        {doc.error_message && (
                          <p className="text-xs text-red-500 mt-0.5 ml-5">{doc.error_message}</p>
                        )}
                      </td>
                      <td className="px-4 py-3 text-text-secondary text-xs capitalize">{doc.document_type?.replace(/_/g, " ")}</td>
                      <td className="px-4 py-3">
                        <span className={`text-xs capitalize font-medium ${statusColor(doc.status)}`}>{doc.status}</span>
                      </td>
                      <td className="px-4 py-3 text-text-secondary text-xs">{doc.tax_year || "—"}</td>
                      <td className="px-4 py-3 text-text-muted text-xs whitespace-nowrap">{formatDate(doc.imported_at)}</td>
                      <td className="px-4 py-3">
                        <button
                          onClick={() => handleDeleteDocument(doc.id)}
                          disabled={deletingId === doc.id}
                          className="p-1.5 text-text-muted hover:text-red-500 rounded transition-colors disabled:opacity-40"
          ...
          <p className="text-xs text-text-muted">Deleting a document record removes the import entry only. Transactions already imported are not affected.</p>
        </div>
      )}
    </div>
  );
